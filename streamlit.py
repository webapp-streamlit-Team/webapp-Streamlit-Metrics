...
if __name__ == "__main__":
    ...
    if df is not None:
        ...
        with tab5:
            ...
            st.write("""Τα ποσοστά που προέκυψαν απο τη ταξινόμηση των δειγμάτων ήταν αισιόδοξα με τον Support Vector Machine να έχει ποσοστό ακρίβειας **έως και 93%** και τον Κ-Nearest Neighbours να έχει ποσοστό ακρίβειας **έως και 100%** σε συνεργασία και με τους reduction αλγόριθμους PCA, UMAP και NMF.""")
            
            st.subheader("Συμπεράσματα και Μελλοντικές προεκτάσεις")
            
            st.write(""" Συμπερασματικά διακρίνουμε ότι πήραμε αρκετά θετικά αποτελέσματα από τους αλγορίθμους ταξινόμησης με υψηλά ποσοστά ακρίβειας που βοηθούν στη καλύτερη ανάλυση και επεξεργασία των δεδομένων. Μελλοντικά θα μπορούσαν να ενταχθούν περισσότεροι αλγόριθμοι ταξινόμησης και αλγόριθμοι συσταδοποίησης για μεγαλύτερη πληθώρα εργαλείων και μεθόδων.""")
        ...
        with tab6:
            ...
            st.write("""Η εφαρμογής μας βοηθά στην οπτικοποίηση, ανάλυση, ταξινόμηση και συσταδοποίηση των δεδομένων από csv ή xlsx αρχεία dataset.""")
            ...
            st.write("""O Καλδάνης Χρήστος ανέλαβε τα εξής: Raw Data, 2D Visualization, Classification Algorithms, Clustering Algorithms, Αναφορά σε **LaTeX**""")
            st.write("""O Λύγκουρης Κωνσταντίνος ανέλαβε τα εξής: UML Diagrams, Συγγραφή των Result και Info tabs. """)
